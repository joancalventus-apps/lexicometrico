...
# Algoritmo simplificado de MTLD
...
if uploaded_file is not None:
    try:
        ...
        if len(all_tokens) == 0:
            ...
            # --- PESTAÑAS ACTUALIZADAS (Cambio de nombre en la 4ta) ---
            ...
            with tab3:
                ...
                - **TTR (Type-Token Ratio):** Riqueza básica. *Sensible a longitud.*
                - **Raíz de Guiraud:** *Más robusta.*
                - **MTLD:** (Measure of Textual Lexical Diversity). *La medida más estable.*
                """)
            ...
            # --- 4. RED DE CO-OCURRENCIAS (Update) ---
            with tab4:
                ...
                try:
                    ...
                    # CÁLCULO DE TAMAÑO DE NODOS PROPORCIONAL A FRECUENCIA GLOBAL
                    # Usamos freq_dist (calculado al inicio) para obtener la frecuencia real de cada palabra en la red
                    # Escalamos: Tamaño base (500) + (Frecuencia * Multiplicador (30))
                    node_sizes = [500 + (freq_dist.get(node, 0) * 30) for node in G.nodes()]

                    fig_net, ax_net = plt.subplots(figsize=(12,8))
                    pos = nx.spring_layout(G, k=0.5, seed=42)
                    
                    # Usamos la lista de tamaños calculada
                    nx.draw(G, pos, with_labels=True, node_color='#aaddff', edge_color='#cccccc', node_size=node_sizes, font_size=10, ax=ax_net)
                    st.pyplot(fig_net)
