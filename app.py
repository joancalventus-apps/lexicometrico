...
# --- 3. CONFIGURACIÓN DE RECURSOS (NLTK y SPACY) ---
LANG_MAP = {'Español': 'spanish', 'Inglés': 'english'}

# A. SOLUCIÓN DEFINITIVA AL ERROR PUNKT_TAB
# Descargamos explícitamente todo lo necesario al inicio
@st.cache_resource
def download_nltk_resources():
    ...
    for res in resources:
        try:
            ...
            nltk.download(res)
        except ValueError: # Para stopwords que no están en tokenizers
            try:
...
# B. CARGA EFICIENTE DE SPACY
@st.cache_resource
def load_spacy_model():
    # Intentamos cargar el modelo pre-instalado via requirements.txt
    model_name = "es_core_news_sm"
    try:
        nlp = spacy.load(model_name)
    except OSError:
        # Fallback de emergencia por si falla requirements.txt
...
def clean_text(text, language='Español', custom_stops=[], min_len=2, apply_lemma=False):
    ...
    if apply_lemma and language == 'Español':
        ...
        # Aumentamos limite de caracteres por si hay textos muy largos
        nlp.max_length = 2000000 
        doc = nlp(str(text).lower())
        # Lematizamos: usamos .lemma_
...
def calculate_mtld(tokens, threshold=0.72):
    def count_factors(token_list):
        ...
        for word in token_list:
            ...
            if ttr < threshold:
                factors += 1; length = 0; word_set = set(); ttr = 1.0
...
use_lemmatization = st.sidebar.checkbox("Aplicar Lematización", help="Agrupa variantes (ej: niños, niñas -> niño). Puede tardar unos segundos.")
...
if uploaded_file is not None:
    try:
        ...
        with st.spinner('Procesando corpus textual (esto puede tardar si usas lematización)...'):
            # Llamada a la función de limpieza con lematización opcional
        ...
        if len(all_tokens) == 0:
            ...
            with tab1:
                ...
                if cur_w:
                    mask = df[text_col].str.contains(cur_w, case=False, na=False) # Buscar en texto original
            ...
            with tab3:
                ...
                with st.info("MTLD: Medida robusta de riqueza léxica (indep. longitud)."): pass
                div_d = []
            ...
            with tab4:
                ...
                st.markdown("**Interpretación:** Nodos = Palabras (Tamaño según frecuencia). Líneas = Co-ocurrencia.")
                ...
                try:
                    ...
                    # Tamaño proporcional
                    node_sizes = [300 + (freq_dist.get(n, 1) * 20) for n in G.nodes()]
